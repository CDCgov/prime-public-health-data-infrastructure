...
def get_blobs(container_url: str, container_prefix: str):
...
def read_fhir_bundles(container_url: str, container_prefix: str) -> Iterator[Tuple[str,dict]]:
...
def store_bundle(container_url: str, prefix: str, bundle: dict, datatype: str) -> None:
    ...
    client = get_blob_client(container_url)
    blob = client.get_blob_client(str(pathlib.Path(prefix) / datatype / f"{uuid.uuid4()}.fhir"))
