...
from IntakePipeline.transform import find_patient_resource, transform_bundle
...
def test_find_patient_record(combined_bundle):
    patient = find_patient_resource(combined_bundle)
    assert patient.get("resource").get("id") == "some-uuid"
...
def test_transform_bundle(patched_geocode, combined_bundle):
    ...
    incoming = find_patient_resource(combined_bundle)
